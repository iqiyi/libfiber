...
// UNICODE 转宽字符
void CWinEchodDlg::Uni2Str(const CString& in, char* buf, size_t size)
{
...
void CWinEchodDlg::OnBnClickedListen()
{
	...
	if (m_fiberListen == NULL)
	{
		...
		char ipbuf[128];
		Uni2Str(m_listenIP, ipbuf, sizeof(ipbuf));
		socket_t lfd = socket_listen(ipbuf, m_listenPort);
	...
	else if (acl_fiber_scheduled())
	{
		GetDlgItem(IDC_LISTEN)->EnableWindow(FALSE);
		acl_fiber_kill(m_fiberListen);
